...
module.exports = class BrowserRunner extends Runner {
    ...
    async run(testCollection) {
        const {desiredCapabilities: {version: defautVersion} = {}} = this._config.forBrowser(this._browserId);

        testCollection.eachTestByVersions(this._browserId, defautVersion, (test) => {
    ...
    async _runTest(test) {
        const browserAgent = BrowserAgent.create(this._browserId, this._browserPool, test);
