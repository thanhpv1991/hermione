...
module.exports = class BrowserRunner extends Runner {
    ...
    async run(testCollection) {
        testCollection.eachTestByVersions(this._browserId, (test, _, browserVersion) => {
            this._running.add(this._runTest(test, browserVersion));
        });
    ...
    async _runTest(test, browserVersion) {
        const browserAgent = BrowserAgent.create(this._browserId, browserVersion, this._browserPool);
