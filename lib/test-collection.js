...
module.exports = class TestCollection {
    ...
    eachTestByVersions(browserId, defaultVersion, cb) {
        const groups = this._specs[browserId].reduce((versions, test) => {
            const version = test.version || defaultVersion || 'unspecified';

            if (!versions[version]) {
                versions[version] = [];
            }

            versions[version].push(test);

            return versions;
        }, {});
        const versions = Object.keys(groups);
        const maxLength = _(versions)
            .map((version) => groups[version].length)
            .max();
        ...
        for (let idx = 0; idx < maxLength; ++idx) {
            for (const version of versions) {
                ...
                if (test) {
                    test.version = test.version || defaultVersion;
